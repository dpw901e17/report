...
The underlying library is partially implemented by Microsoft in a closed common runtime, which communicates with the \gls{GPU} driver through a device \gls{DDI}.
On the other hand, OpenGL is designed by several hardware and software vendors in coorperation, and requires that manufacturers write their own library from scratch. \cite{wikiDXOpenGLCompare}
...
Exemplifying this, Wikipedia lists a total of \~200 games with OpenGL support \cite{wikiOpenGLGames}, while a total of \~500 games is listed with Direct3D 11 support \cite{wikiDX11Games}. 
...
This includes Valve’s Source engine \cite{sourceEngine}, the Unity engine \cite{ unityEngine} and Epic Game’s Unreal engine\cite{unrealEngine}.
...
Not much solid academic work has been published comparing OpenGL and Direct3D.
...
Vulkan seems to be up for the task, already being supported on many \glspl{GPU} \cite{wikiVulkan}.
