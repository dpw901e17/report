...
\todo{Didn't they also want a better API for mobile phones? (whenever they show off the power of Vulkan, it was on phones during the early days of Vulkan}
...
\todo{"avoid" did you mean "avoids"? && "when it comes to when the commands" replace one 1 "when" please }
...
\todo{Perhaps we could add which of the oldest generation GPU from Nvidia and AMD cards supports Vulkan.}
...
\todo{I would rather use the word annouced rather than launched}
...
\todo{alot more general than DX12, but easier to follow. Not sure if this is good enough or if we need to go more in depth like with DX12}
