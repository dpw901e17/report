...
An \gls{API} is an interface, which developers use to access some underlying code library. \todo{Maybe introduce APIs earlier?}
...
A benefit of OpenGL is that manufacturers may provide extensions to the \gls{API} to give developers access to new hardware features \cite{openGLExtensions} \todo{Is this always a benefit?}.
...
This seems to indicate that Direct3D is a defacto-standard for graphics in games. \todo{Ifølge Bent er dette ren spekulation, needs fixin'}
...
Examples include a performance benchmark between OpenGL 3.2 and Direct3D 12, \glspl{API} released 6 years apart and with different underlying design philosophies \todo{What philosophies?} \cite{geek3DBenchmark}.
